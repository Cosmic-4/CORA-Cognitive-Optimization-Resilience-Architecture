...
export const DashboardView = ({ metrics, incidents, onOpenDeployModal, onMutateWeb, pipelineActive, pipelineStage }: DashboardViewProps) => {
  ...
  // anime: entrance + staggered cards
  useEffect(() => {
    if (!rootRef.current) return;
    const els = rootRef.current.querySelectorAll('.dash-anim');
    animate(els, {
      translateY: [16, 0],
      opacity: [0, 1],
      duration: 520,
    ...
    // pipeline pulse
    if (pipelineActive) {
      const dots = rootRef.current.querySelectorAll('.pipeline-dot');
      animate(dots, {
        scale: [1, 1.25, 1],
        duration: 700,
        loop: true,
  ...
  // resilience number count-up + bar fill
  useEffect(() => {
    if (!resilienceRef.current) return;
    const num = resilienceRef.current.querySelector('.resilience-num');
    const bar = resilienceRef.current.querySelector('.resilience-bar') as HTMLElement;
    if (num) {
      animate(num, {
        // anime text scramble via opacity + scale
        scale: [0.92, 1],
        opacity: [0.7, 1],
        duration: 420,
        ease: 'outExpo',
    ...
    if (bar) {
      ...
      // keep width via scaleX, set actual width before
      bar.style.width = `${metrics.autoResolutionProgress}%`;
      animate(bar, { scaleX: [0.7, 1], duration: 600, ease: 'outExpo' });
    }
  }, [metrics.autoResolutionProgress]);
      ...
      <div className="dash-anim flex flex-col items-center gap-5 mt-2 mb-10">
        <button
          onClick={(e) => {
            ...
            animate(b, { scale: [1, 0.97, 1], duration: 240, ease: 'outExpo' });
            onMutateWeb();
          }}
        ...
        <div ref={resilienceRef} className="dash-anim rounded-[18px] p-6 flex flex-col" style={{ background: 'var(--color-bg-elevated)', border: '0.5px solid var(--color-border-subtle)', boxShadow: '0 1px 3px rgba(0,0,0,0.04), 0 8px 24px rgba(0,0,0,0.06)' }}>
          ...
          <div className="flex-1 flex flex-col items-center justify-center py-6">
            ...
            <div className="w-full h-2 rounded-full mt-6 overflow-hidden" style={{ background: 'var(--color-bg-secondary)', border: '0.5px solid var(--color-border-subtle)' }}>
              <div className="resilience-bar h-full rounded-full origin-left" style={{ width: `${metrics.autoResolutionProgress}%`, background: 'var(--color-cora)' }} />
            </div>
          </div>
